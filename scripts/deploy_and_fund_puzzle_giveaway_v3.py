...
def default_token(provider) -> str:
    if token := os.environ.get("PUZZLEGIVEAWAY_V3_TOKEN"):
        return token
...
def validate_game_values(
    ...
    if cliff_seconds < 0:
        raise click.BadParameter("cliff-seconds cannot be negative")

    if curve_exponent not in (2, 3):
        raise click.BadParameter("curve-exponent must be 2 or 3")
    ...
    if floor_amount < max_amount and now + cliff_seconds >= deadline:
        raise click.BadParameter("deadline must be after the cliff window")
...
def echo_contract_state(contract):
    ...
    click.echo(f"deployed_cliff_seconds={contract.cliff_seconds()}")
    ...
    click.echo(f"deployed_claimable_amount={contract.claimable_amount()}")
...
@click.option(
    "--token",
    envvar="PUZZLEGIVEAWAY_V3_TOKEN",
    default=None,
    help="ERC20 prize token address.",
)
@click.option(
    "--refund-to",
    envvar="PUZZLEGIVEAWAY_V3_REFUND_TO",
    required=True,
    help="Address that receives clawback funds.",
)
@click.option(
    "--prompt",
    envvar="PUZZLEGIVEAWAY_V3_PROMPT",
    required=True,
    help="Public puzzle prompt shown by the contract.",
)
@click.option(
    "--max-amount",
    envvar="PUZZLEGIVEAWAY_V3_MAX_AMOUNT",
    required=True,
    type=int,
...
@click.option(
    "--floor-amount",
    envvar="PUZZLEGIVEAWAY_V3_FLOOR_AMOUNT",
    required=True,
    type=int,
    help="Starting prize in token base units after the game begins.",
)
@click.option(
    "--deadline",
    envvar="PUZZLEGIVEAWAY_V3_DEADLINE",
    required=True,
    type=int,
    help="Unix timestamp when answering expires.",
)
@click.option(
    "--cliff-seconds",
    envvar="PUZZLEGIVEAWAY_V3_CLIFF_SECONDS",
    default=0,
    show_default=True,
    type=int,
    help="Seconds after start where claimable amount stays at the floor.",
)
@click.option(
    "--curve-exponent",
    envvar="PUZZLEGIVEAWAY_V3_CURVE_EXPONENT",
    default=2,
    show_default=True,
    type=int,
    help="Convex ramp exponent: 2 for quadratic, 3 for cubic.",
)
@click.option(
    "--answer-hash",
    envvar="PUZZLEGIVEAWAY_V3_ANSWER_HASH",
    required=True,
...
def cli(
    ...
    click.echo("PuzzleGiveawayV3 deployment and funding")
    ...
    click.echo(f"cliff_seconds={cliff_seconds}")
    ...
    click.echo(f"puzzle_giveaway_v3={contract.address}")
