...
"""
@title KingOfTheHillGiveawayV52
@notice Limited-shot ERC20 king-of-the-hill giveaway with a hidden side quest boost.
"""
...
original_deadline: public(uint256)
deadline: public(uint256)
max_deadline: public(uint256)
extension_window: public(uint256)
max_overtime: public(uint256)
...
side_quest_hash: bytes32
side_quest_boost_bps: uint256
side_quest_used: HashMap[address, bool]
...
def __init__(
    ...
    _max_overtime: uint256,
    _max_shots: uint256,
    _curve_exponent: uint256,
    _answer_hash: bytes32,
    _side_quest_hash: bytes32,
    _side_quest_boost_bps: uint256
):
    """
    ...
    @param _extension_window Minimum response window after late shots.
    @param _max_overtime Maximum seconds that late shots can extend the live deadline.
    ...
    @param _side_quest_hash Hash of the hidden side quest answer string.
    @param _side_quest_boost_bps One-time side quest hold-time boost, in BPS of game duration.
    """
    ...
    assert _extension_window > 0, "bad extension"
    assert _max_overtime >= _extension_window, "bad overtime"
    ...
    self.max_deadline = _deadline + _max_overtime
    self.extension_window = _extension_window
    self.max_overtime = _max_overtime
    self.max_shots = _max_shots
    self.curve_exponent = _curve_exponent
    self.answer_hash = _answer_hash
    self.side_quest_hash = _side_quest_hash
    self.side_quest_boost_bps = _side_quest_boost_bps
...
def _reign_hold(_since: uint256, _until: uint256) -> uint256:
    ...
    effective_until: uint256 = _until
    if effective_until > self.original_deadline:
        effective_until = self.original_deadline

    if effective_until < _since:
        return 0

    return effective_until - _since
...
def _reign_prize(_since: uint256, _until: uint256) -> uint256:
    ...
    if effective_until < _since:
        return self._prize_for_hold(0)

    return self._prize_for_hold(effective_until - _since)


@internal
def _extend_deadline_if_needed(_shot_time: uint256):
    time_left: uint256 = self.deadline - _shot_time
    if time_left >= self.extension_window:
        return

    old_deadline: uint256 = self.deadline
    new_deadline: uint256 = _shot_time + self.extension_window
    if new_deadline > self.max_deadline:
        new_deadline = self.max_deadline
...
def _record_current_reign(_ended_at: uint256):
    ...
    is_side_quest: bool = (
        self.side_quest_hash != empty(bytes32)
        and self.king_answer_hash == self.side_quest_hash
    )
    if self.king_answer_hash != self.answer_hash and not is_side_quest:
        return
    ...
    if (
        is_side_quest
        and not self.side_quest_used[self.king]
        and self.side_quest_boost_bps > 0
    ):
        correct_hold += self.game_duration * self.side_quest_boost_bps // BPS
        self.side_quest_used[self.king] = True

    elapsed_cap: uint256 = self._reign_hold(self.start_time, _ended_at)
    if correct_hold > elapsed_cap:
        correct_hold = elapsed_cap
    ...
    if prize_amount > self.candidate_prize_amount:
...
def start_game():
    ...
    assert not self.ended, "ended"
    assert block.timestamp < self.original_deadline, "expired"

    self.started = True
    self.start_time = block.timestamp
    self.game_duration = self.original_deadline - block.timestamp

    log GameStarted(start_time=block.timestamp, deadline=self.original_deadline)
...
def shoot(_answer: String[128]):
    ...
    self._extend_deadline_if_needed(block.timestamp)
...
def finalize():
    """
    @notice Ends the game after expiry and pays the best banked correct holder.
    """
    assert self.funded, "not funded"
    assert self.started, "not started"
    assert not self.ended, "ended"
    ...
    self._record_current_reign(self.original_deadline)
    self.ended = True
...
def king_prize() -> uint256:
    ...
    return self._reign_prize(self.king_since, block.timestamp)
